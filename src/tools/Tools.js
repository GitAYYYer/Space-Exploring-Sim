function PickRandomJsonObjKey(jsonObj, key){
    let obj_keys = Object.keys(jsonObj);
    return obj_keys[key];
}

//thanks https://github.com/hbi99/namegen
//returns a list, not necessary will change eventually
function Namegen(count, planetSeed) {
    let r = new Random(planetSeed);
    var vowels = {  '1': ["b", "c", "d", "f", "g", "h", "i", "j", "k", "l", "m", "n", "p", "q", "r", "s", "t", "v", "w", "x", "y", "z"],
        ...
        mtx = [[1,1, 2,2, 5,5],
            ...
            [3,3, 4,4, 1,1, 4,4, 5,5],
            [4,4, 1,1, 4,4, 3,3, 6,6]],
        fn = function(i) { return Math.floor(r.nextFloat() * vowels[i].length); },
        ret = [],
        name,
        comp,
        i, il,
    ...
    for (; c<count; c++) {
        ...
        ret.push(name.charAt(0).toUpperCase() + name.slice(1));
    }
